...
* DB ERD 수정 (4차)
  * USER (회원정보) 테이블
    * 외래키 삭제
      * 이유 : 성능 및 속도 이슈 및 테이블 구조 변경 시 고려사항 문제로 인해 외래키 삭제
    * 프로필 사진 파일 관리를 위해 새로운 테이블 생성 (PROPILE_IMAGE)
      * 이유 : IMG의 경우 조회시 트래픽 발생할 가능성이 많아 전용으로 관리하기 위해 
    * user_id 컬럼 제거
      * 이유 : 소셜로그인, 자체로그인 모두 email이 기준이기 때문에 제거
  * USER_FOLLOWER, USER_FOLLOWING (팔로우&팔로잉) 테이블 통합
    * 이유 : 같은 기능을 하나 나를 팔로우했냐? 내가 팔로우했냐의 차이이므로 통합
    * SEQ 생성
      * 이유 : 성능 및 속도 이슈로 인해 INT형 SEQ 생성
  * LIKE_COMMENT (좋아요&댓글) 테이블 분리
    * 좋아요 & 댓글 테이블을 통합하여 함께 데이터 조회시 속도이슈 생길 가능성이 있음
      * 좋아요(POST_LIKE)테이블, 댓글테이블(COMMENT_LIKE) 생성
    * 비식별 관계 해제
      * 이유 : 기존의 테이블 경우 하나의 좋아요에 여러 게시물이라는 다대다 관계므로 하나의 게시물에 좋아요가 여러개인 관계형성을 위해
    * 좋아요 갯수는 기본키가 될수 없으로 인덱스가 자동증가 되는 SEQ 생성
    * 게시물의 경우 최초등록 DATE는 존재하지만 수정시의 DATE가 없어 추가
  * POST (게시물) 테이블 분리
    * 동영상 파일과 이미지 파일을 관리하는 각 테이블 생성 
      * POST_VIDEOS(동영상파일관리테이블), POST_IMAGE(이미지파일관리테이블)
      * 이유 :  프로필 관련 부분과 같은 이유로 트래픽이 발생할 수 있으므로 각 파일별로 전용으로 관리 하기 위해 테이블 분리
  * 전체적인 COMMENT 추가
...
* 쿠키와 세션을 사용하는 이유
  ...
  * HTTP 프로토콜의 특징 
    * Connectionless 프로토콜 (비연결 지향)
       1) 클라이언트가 서버에 요청 했을때 요청에 맞는 응답을 보낸 후 연결을 끊는 처리 방식
    * Stateless 프로토콜
       2) connection을 끊는 순간 클라이언트와 서버 통신이 끝나며 상태 정보는 유지하지 않음
  * 서버와 클라이언트가 통신할때 연속적으로 이어지지 않고 한번의 통신 후에 끊어짐 --> 서버는 클라이언트가 누구인지 지속적인 인증 필요 따라서 쿠키와 세션을 이용하여 불편함 감소

* 쿠키(Cookie)란?
  * 가장 큰 특징은 클라이언트에 저장
  * 사용자가 사이트 방문 시 서버에서 사용자의 컴퓨터에 저장하는 작은 기록 정보 파일
* 쿠키(Cookie) 특징
  * 이름, 값, 저장기간, 경로 정보로 구성
  ...
  * 하나의 쿠키는 4KB까지 저장 가능
* 쿠키(Cookie) 동작 흐름
  * 클라이언트 페이지 요청 —> 웹 서버에서 쿠키 생성 --> 생성한 쿠키에 정보를 담아 HTTP화면을 돌려줄때 클라이언트에게 돌려줌 --> 돌려받은 쿠키는 클라이언트가 가지고 있다 다시 서버에 요청 시 요청과 함께 쿠키 전송
  * Ex) 방문 사이트에서 로그인 시 “아이디와 비밀번호를 저장하시겠습니까?” , 팝업창을 통해 “오늘 이 창을 다시 보지 않기” 체크

* 세션(Session)이란?
  * 가장 큰 특징은 서버에 저장하며 방문자가 웹 서버에 접속해 있는 상태를 하나의 단위로 보는 것
  * 일정 시간 동안 같은 사용자로부터 들어오는 일련의 요구를 하나의 상태로 보고 그 상태를 유지시키는 기술
  * 일정 시간이란 방문자가 웹 서버에 접속한 시점부터 종료하여 연결을 끝내는 시점
